...
module.exports = {
  ...
  dialogFlowSessionLanguageCode:
    process.env[process.env.ENV + "_DIALOG_FLOW_SESSION_LANGUAGE_CODE"],
  googleClientEmail: process.env[process.env.ENV + "_GOOGLE_CLIENT_EMAIL"],
  googlePrivateKey: JSON.parse(
    `"${process.env[process.env.ENV + "_GOOGLE_PRIVATE_KEY"]}"`
  ),
  tmdbAccessToken: process.env[process.env.ENV + "_TMDB_ACCESS_TOKEN"],
  tmdbApiKey: process.env[process.env.ENV + "_TMDB_API_KEY"]
};
